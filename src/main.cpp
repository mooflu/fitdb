...
public:
	...
	void OnMesg( fit::RecordMesg& record ) override
	{
		// fmt::print( "Record:\n" );
		if (
			!record.IsPositionLatValid() ||
			!record.IsPositionLongValid()
		) {
			return;
		}
		...
		if (rec.lat < swcLat) swcLat = rec.lat;
		if (rec.lng < swcLng) swcLng = rec.lng;
		if (rec.lat > necLat) necLat = rec.lat;
		if (rec.lng > necLng) necLng = rec.lng;
...
select 1 from activity where garminActivity=@garminActivity;
	...
	char *zErrMsg = 0;
	std::string query = fmt::format(R"sql(
...
limit {};
	...
	fmt::print("query: {}\n", query);
	fmt::print("Results:\n");
	int rc = sqlite3_exec(db, query.c_str(), [](void *_, int argc, char **argv, char **azColName) -> int {
...
int main(int argc, char *argv[])
{
	...
	cli.add_argument("--nec")
		.help("north east corner")
		.nargs(2)
		.scan<'g', double>();

	std::vector<double> swc{0.0,0.0};
	cli.add_argument("--swc")
		.help("south west corner")
		.nargs(2)
		.scan<'g', double>();
	...
	int limit = 100;
	cli.add_argument("-l", "--limit")
		.help("max number or records to return")
		.default_value(limit)
		.store_into(limit);
